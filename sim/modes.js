...
function stepSettings(title, pressed) {
  ...
  if (s.page === null) {
    ...
    if (pressed('confirm')) {
      ...
      //
      // This row was a reserved stub that played the error sound, and the
      // name was never decoration: the seven attacks the real selector can
      // never reach are labelled UNUSED wherever the player meets them, and
      // KAIZO is where that cut content finally goes.
      //
      // It returns an INTENT, exactly as SHARE and the credits links do —
      // `sim/` has no DOM and a headless verifier has to be able to walk
      // this path. The driver owns the navigation, which is also what keeps
      // the isolation contract intact: the main page LINKS to the kaizo
      // page, it never imports from kaizo/ (kaizo/HANDOFF.md §2), so the
      // whole main build still works with kaizo/ deleted — the link simply
      // has nowhere to go.
      if (page === 'unused') {
        out.kaizo = true;
        out.selected = true;
...
export function stepTitle(title, input, attacks) {
  ...
  if (title.settings) {
    ...
    // A WHITELIST, so a new intent must be added HERE too. `kaizo` was set
    // correctly inside stepSettings and silently dropped on the way out —
    // the settings page reported "selected" and the driver saw nothing to
    // act on, which looks exactly like a dead button.
    return {
      moved: r.moved, chosen: false, selected: r.selected, error: r.error,
      link: r.link ?? null, share: r.share ?? false, kaizo: r.kaizo ?? false,
    };
  }
