// KAIZO KNIGHT — the kaizo turn loop. A KAIZO-OWNED COPY of
// sim/scenes/practice.js, parametrized by schedule table.
//
// WHY A COPY (kaizo/HANDOFF.md §2 + K3): the practice director reads the real
// fight's schedule constant lexically, and its director/turnClock objects are module-private,
// so composition cannot inject a different schedule — and patching sim/ for
// kaizo's sake is forbidden by the isolation contract. Copying INTO kaizo/ is
// the sanctioned fallback. This file imports from sim/ only (one-way); nothing
// under sim/, render/, web/ or tools/ may import from it.
//
// WHAT IS DIFFERENT from the original, exhaustively:
//   * every read of the original's imported schedule constant is `e.table` —
//     the director carries the schedule
//     (KAIZO_TABLE via buildKaizoScene), so V-B/V-C variants reuse this driver
//   * nextTurn is table-parametrized (kaizoNextTurn below, same control flow)
//   * the phase label is prefixed KAIZO — nothing invented ships unlabelled
//   * launches are recorded in state.kaizo.launched so verify-kaizo can assert
//     the table is what ACTUALLY runs, not just what the HUD says
//   * the two phase-length literals (phase-3 latch, phase-4 resume) read the
//     table's own row counts instead of 4 and 2
// Everything else is the verified turn machinery, byte-for-byte. The entity
// names fight_director / turn_clock are KEPT: clearTurn's SURVIVES_TURN
// keep-list (sim/scenes/fight.js) sweeps anything not named there.
//
// The attack ENGINES this drives are the verified ones; the SCHEDULE is
// invented. That split is the whole fidelity claim of KAIZO v-A.
//
...
// the module that owns the tables and the version registry.

/**
 * sim/scenes/fight.js's nextTurn, table-parametrized — same control flow:
 * phase 3 loops onto itself, phase 4 falls back into the phase-3 loop
 * (ROARING sets `phase = 3`), phases 1/2 hand forward.
 */
...
/**
 * obj_battlecontroller's Alarm_2, whole:
 *
 *     reset = 0;
 *     scr_mnendturn();          // global.mnfight = 0; global.myfight = 0;
 *
 * `mnfight = 0` is the ONLY thing the trace can see, and `clockOn` is what
 * the harness reads it off, so this is where `clockOn` drops — fifteen frames
 * after the sweep, not on it.
 *
 * THE SCHEDULE STEP RIDES WITH IT. `attackchoice` / `difficulty` are latches
 * the harness reads off the director's live row while `clockOn` holds, and the
 * recording keeps the FINISHED turn's values through the whole window (f657-671
 * read ac 0, the crescent's, and only turn 3's selector at f780 writes 2). So
 * the row advance computed on the sweep frame is parked in `pendingTurn` and
 * applied here — the sweep's own side effects (the phase-4 gate, the turn-end
 * message, `rotatingslash3used`) stay on the sweep frame, where the knight's
 * `turntimer <= 1` block genuinely runs them.
 */
...
const turnClock = {
  ...
  step(e, state) {
    // obj_battlecontroller's ALARM[2], and it fires in the ALARM SLOT — the
    // frame's first thing, before any Step. GameMaker runs Begin Step,
    // Alarms, then Step; this entity is stepOrder -100, so its `step` is the
    // earliest slot this engine offers and everything that reads `clockOn`
    // (the End Step decrement below, the director's endStep) sees the alarm's
    // result within the same frame.
    //
    //     // Step_0:1477-1500, under `mnfight == 2 && timeron == 1`
    //     if (global.turntimer <= 0 && reset == 0) {
    //         with (obj_bulletparent) instance_destroy();   // the SWEEP
    //         ...
    //         reset = 1;
    //         if (noreturn == 0) alarm[2] = 15;
    //     }
    //     // Alarm_2:  reset = 0; scr_mnendturn();          // mnfight = 0
    //
    // MEASURED, ALL 32 BOUNDARIES IN _tok3: `mnfight` stays 2 for exactly 15
    // frames after `turntimer` first reads <= 0, every single turn (247->262,
    // 657->672, 1045->1060, ... 12289->12304). Constant, never drifting — the
    // literal 15. The sweep and the turn's END are two events fifteen frames
    // apart, and this scene used to collapse them into one: `clockOn` cleared
    // on the sweep frame, so the trace read mnfight 0 from f657 where the
    // recording reads 2 through f671.
    //
    // `reset == 1` is why the sweep cannot re-arm during those 15 frames;
    // here that is `e.started === false`, which gates the director's whole
    // finish block.
  ...
  endStep(e, state) {
    ...
    //
    // NO `> 0` FLOOR. The controller's decrement has no lower bound — its
    // only gate is `mnfight == 2 && timeron == 1`, and both hold through the
    // whole alarm[2] window, so the clock keeps falling while the board
    // closes. The recording is explicit: turn 2 reads -0.2333 at f657 and
    // walks to -14.2333 at f671, then FREEZES there for the party phase (the
    // alarm has set mnfight 0 by f672, so the block stops running). The old
    // floor pinned the sim at the expiry frame's value for all fifteen.
    const d = e.director;
...
const director = {
  ...
  create(e, state) {
    // VERSION HOOKS (V-C etc.): every hook defaults to the vanilla-shaped
    // behavior below, so a hookless build (V-A) is byte-identical.
    if (!e.hooks) e.hooks = {};
    e.phase = 1;
    e.turn = 0;
    ...
    e.started = false;
    e.clockOn = false;
    // obj_battlecontroller's alarm[2], unarmed (GameMaker's alarms rest at
    // -1; a plain 0 reads the same here because the countdown is `> 0`).
    e.endAlarm = 0;
    e.pendingTurn = null;
    e.menuShown = false;
    e.soulHold = null;
  ...
  endStep(e, state) {
    ...
    // KAIZO_MENU_DEBUG — one guarded line per frame of the party phase, for
    // aligning the sim's menu sub-state against a recording. The whole-fight
    // trace carries only mnfight/turntimer/rtimer through the menu, so the
    // sub-state has to come from this side; CLAUDE.md's "Instrument before
    // theorising". This is what settled the CrescentSlash -12 (see the note
    // over the bar's creation below). Diagnostic only, no gameplay effect.
    ...
    if (e.pendingSwing) {
      for (const s of e.pendingSwing) {
        ...
        if (dealt > 0) {
          ...
          // A BLOCKED SWING RAISES NO IMPACT -- and therefore no screen shake.
          // obj_heroparent's attack block (Other_10:102-111, Step_0:432-438) is
          // wrapped in `if (knightblock == 0)`, so when the knight's guard is up
          // the game creates no attack object, plays no snd_impact, and (for
          // Susie, the only hero whose swing shakes) creates no obj_shake. The
          // sim raised one on any `dealt > 0`, and a blocked hit still deals
          // ceil(damage / 5).
          //
          // MEASURED with the recorder's new camera log (KAIZO_ORACLE_CAMERA=1)
          // against the tracer's --cameralog: over the probe fight's first 2600
          // frames the two cameras agree everywhere EXCEPT eight frames, all of
          // them phantom shakes in the sim during the menu phase -- f323-f327 and
          // f720-f721, where the game holds (0, 0) with no obj_shake alive and
    ...
    if (e.started) {
      ...
      // `reset = 1` — the sweep is one-shot, and `e.started` is this scene's
      // form of it. `clockOn` and `arenaOpen` do NOT drop here: mnfight stays
      // 2 and the board is still on screen (closing) until alarm[2] fires.
      e.started = false;
      e.balloonDone = false;
      ...
      if (state.soul) {
        // AND IT NEVER RUNS ITS LAST DECREMENT. `global.inv -= 1` is
        // obj_heart's own Step (Step_0:249) and the sweep above is
        // obj_battlecontroller's Step, and GameMaker walks an event BY OBJECT
        // INDEX: obj_battlecontroller is 1393, obj_heart 1462, so the
        // controller destroys the heart before the heart's Step exists to run.
        // The heart's last frame costs no inv.
        //
        // This engine puts the decrement in the soul's `motion` slot, which
        // runs BEFORE this end step, so by the time the sweep lands the
        // decrement has already happened and has to be given back. Same
        // compensation family as soulPrev and grazePrev: the sim's phase
        // order is fixed and the sites that straddle it pay it back locally.
        //
        // MEASURED on _tok3 f3945, the frame atk_Frenzy1 releases the clock
        // (turntimer 999767.97 -> -2) and the sweep fires: the recording holds
        // inv at -159 from f3944 while the sim ran on to -160 and stayed one
        // behind for the rest of the fight, since nothing resets inv until the
        // next hit. It was the canonical trace gate at f3945.
        state.invTimer += 1;
        state.soul.alive = false;
        state.soul = null;
      }
      // THE BOARD CLOSES ON THE SWEEP, over the alarm's own fifteen frames.
      // The sweep sets `with (obj_darkener) darken = 0`, and the darkener's
      // Draw is what reaches the box (obj_darkener Draw_0:23-28):
      //
      //     if (darken == 0) { with (obj_growtangle) growcon = 3; ... }
      //
      // obj_growtangle's Step then runs `timer -= 1` with `maxtimer == 15`
      // (Create_0:5), so the scale walks to zero in fifteen steps and the
      // instance destroys itself. MEASURED at both boundaries in _tok3: the
      // box holds 0.8 on the sweep frame (f657), reads 0.7466666698 on f658 —
      // 0.8 * 14/15 exactly — steps down to 0.0533333346 on f671 and is GONE
      // on f672. Turn 1's 2.24 x 1.76 box does the same from f248. This scene
      // left the box parked at full size for the whole party phase.
      ...
      //
      // The counter is decremented in turnClock's `step` — the alarm SLOT —
      // so `clockOn` has already dropped by the time this endStep and the End
      // Step decrement run on the firing frame. See fireTurnEndAlarm.
      e.endAlarm = 15;
      e.gap = 15;
      e.spawnDelay = RTIMER_SPAWN;
      e.turnsRun += 1;
      ...
      if (!e.hooks.advance) {
        // (Under a custom advance hook the hook owns the turn-end message —
        // this whole vanilla flavour-line block is skipped.)
      ...
      // A CUSTOM ADVANCE (V-C: the mod's nextAttack chain, its 60% gate, the
      // AfterFinal resume-replay) owns the whole schedule step INCLUDING the
      // turn-end message; the vanilla tail below is skipped entirely.
      if (e.hooks.advance) {
        const r = e.hooks.advance(state, e, { prevPhase, prevTurn });
        // PARKED, NOT APPLIED — alarm[2] moves the row. See fireTurnEndAlarm:
        // the harness latches `attackchoice` / `difficulty` off this row for
        // as long as `clockOn` holds, and the recording keeps the finished
        // turn's values through the whole fifteen.
      ...
      // PARK THE ROW, same rule as the hook path above: everything this block
      // decided stands, but `e.phase` / `e.turn` do not move until alarm[2]
      // fires. The vanilla tail mutates them in place while it works (the
      // phase-4 gate reads `e.phase` after the natural step), so the parking
      // is done at the end and the live pair is rewound.
    ...
    const stepTalkWriter = () => {
      ...
      // THE KAIZO SAVE HAS TEXT AUTO-ADVANCE **OFF**, and that is a different
            // setting from the vanilla reference save this line was inherited from.
            //
            // `global.flag[10]` is the text automash setting. With it ON, every raw
            // `button3_h()` runs the writer automash: SKIP the typing to the end,
            // then DISMISS on the next toggle. The tok3 token holds button3 on 3252
            // of 13000 frames, so with automash on the sim tears through every
            // balloon and the whole enemy-talk phase collapses.
            //
            // MEASURED, against the tok3 recording: the mod spends **420 frames** in
            // `mnfight == 1` across the fight, in nine holds of 10-74 frames. With
            // automash on the sim spent **116**; with it off, **463**. Nine of nine
            // of the mod holds end 0-1 frames after a button3 EDGE, at a constant
            // offset across holds ranging 10 to 74 frames -- so the hold is ended by
            // the PLAYER PRESS through the `(button3_p() && talktimer > 15)` arm,
            // not by the writer finishing. Automash bypasses that arm entirely.
            //
            // THIS IS INFERRED FROM THE FRAME BUDGET, NOT READ OFF THE SAVE. The
            // vanilla whole-fight recorder logs `global.flag[10]` in its trace
            // (oracle_fullfight.csx); oracle_kaizo_fight.csx does NOT, so the kaizo
            // side has never measured it. Adding that column is what would settle
            // it, and the remaining +43 frame overshoot is the reason to bother.
            // The RECREATION lanes set state.textAutoMash = false explicitly in
            // kaizo-fight.js (v.knight); every other version keeps vanilla default-ON,
            // so the playable V-A mode is untouched by the measurement.
      ...
      // THE KNIGHT KILLS THE BALLOON HIMSELF, and this is his arm, not a
      // stand-in: obj_knight_enemy Step_0:368-375
      //   if (talked == 0.6) { talktimer++;
      //     if ((button3_p() && talktimer > 15) || !i_ex(obj_writer)) {
      //       with (obj_writer) instance_destroy(); ...the reply... } }
      // MEASURED in the writer sidecar (_writer recording, balloonturn 7): the
      // line "Thing is,&you actually.../%" is still crawling at pos 19 of 27
      // with halt 0 on f2472, and on f2473 -- a b3 frame, talktimer 17 -- it is
      // gone and the 50-character reply exists with talked 0.7. No button could
      // have closed it (the close needs halt != 0), so the arm did.
      //
      // Balloonturn 6 goes the OTHER way in the same recording: talktimer only
      // reaches 10, no b3 press qualifies, and the writer dies to the ordinary
      // confirm on f2044. Both paths are real; the sim needs both.
      // THE RECORDER PRE-EMPTS A REPLY WHOSE FIRST WRITER DIES ON ITS OWN.
      // 
      // This is a HARNESS artifact, not the mod, and it is emulated here the same
      // way --keep-alive emulates the recorder HP pin: only when the tracer sets
      // state.recorderTalkSkip, never in play. oracle_kaizo_fight.csx carries a
      // stall-guard in obj_time Draw_0:
      // 
      //     if (global.mnfight == 1 && !i_ex(obj_writer))
      //         with (obj_knight_enemy) if (talked >= 0.5) talked = 1;
      // 
      // It runs at the END of the frame the balloon writer dies on, so the knight
      // next Step sees talked == 1, never reaches the 0.6 block, and the reply is
      // never created. It only bites when the writer dies BY ITSELF before a
      // button3 press lands: an 8-character line (Heheh..., balloonturn 6) finishes
      // typing before talktimer can pass 15. Every longer line is dismissed by the
      // press first, inside the knight own Step, so its reply is born before the
      // guard looks -- which is why holds 1..8 of _tok3 carry replies and hold 0
      // does not (10 frames; one writer, one balloon in the seq log, verified by
      // re-recording with obj_writer and obj_battleblcon watched). The real mod
      // plays that reply; a recording made with KAIZO_ORACLE_TALKSKIP=0 would show
      // it and would then need this off.
      ...
      if (dismissed) {
        if (dlg.speaker === 'knight' && dlg.ballooncon && !harnessKilledReply) {
          ...
          //
          // UNLESS THE RECORDER'S GUARD PRE-EMPTED THE REPLY. The stall-guard
          // runs at the END of the death frame (obj_time Draw_0: `mnfight == 1
          // && !i_ex(obj_writer)` -> `talked = 1`), so the knight's very next
          // Step reaches `talked == 1 && !i_ex(obj_writer)` and mnfight moves
          // to 1.5 -- no alarm[6], no linger. MEASURED, _tok3 hold 0 (balloonturn
          // 6, "Heheh.../%"): the writer dies on the f2056 confirm press in the
          // recording and here alike, and the recording flips at f2057 where
          // this step, having only cleared, flipped at f2058. The step that
          // sees the death IS the knight's next Step: fall through to the
          // selector below instead of returning.
          e.talkWriter = null;
          clearDialogue(dlg);
          e.talkTimer = 0;
      ...
      // THE AUTO-ADVANCE RUNS, AND ITS SKIP DOES NOTHING TO A BALLOON.
      // Same recording, same phase: automash_timer toggles 0 -> 1 on the b3
      // frame f2037 and 1 -> 0 on f2041, and prevent_mash_buffer reads 2 on
      // each of those frames (set to 3, decremented once in the same Draw) --
      // so global.flag[10] IS on in the reference save. Yet pos still
      // advanced one per frame across f2037, where the toggle to 1 presses
      // button2: the balloon never jumped to length + 1 the way the battle-
      // message writers in the same file do (phases 0-4, pos at length + 1 on
      // their first row). So the skip is unavailable to this writer and only
      // the buffer matters -- which is what closes the phase two frames later
      // than the sim used to: the confirm on f2042 is blocked at pmb 1, and
      // f2044 (pmb -1) is the press that kills it. mnfight flips f2045.
      if (flag10 && b3Held) {
        w.pmb = 3;
        w.automash = w.automash === 0 ? 1 : 0;
        if (w.automash === 0) b1 = true;
        // NO b2 HERE. The toggle to 1 does press button2 in the game, but a
        // balloon writer is not skippable, so modelling the press as a skip
        // is what a translation would get wrong, not the press itself.
      }
      ...
      // A CLOSABLE-AGE GUARD WAS TRIED HERE AND REVERTED (2026-09-02). _probeall's
      // Tunnel 1 -> Rising Abyss B transition ran two frames longer in the game
      // (mnfight 1 -> 2 at f2045; the sim, halting its 8-character line at f2041,
      // closed on the f2042 press and flipped at f2043). Closing only from the
      // second step after the halt fixed that fight (f2043 -> f2229) and broke
      // _tok3's SAME transition (byte gate f2591 -> f2057), so the game's writer
      // does not close later in general: the likelier cause is a different, two-
      // character-longer line in the probe fight (the mod picks by state the
      // director's table does not track). Open lead, ledger addendum.
      if (w.halted) w.haltAge = (w.haltAge ?? 0) + 1; // kept as a diagnostic field
      // THE TERMINATOR COSTS A FRAME. obj_writer types from Alarm_0, which
      // reads the character AT pos and then does pos += 1; the halt is set on
      // the frame it reads the "/" -- one PAST the last visible character.
      // MEASURED, the recorder's writer sidecar (KAIZO_ORACLE_WRITERLOG=1,
      // _writer recording): balloonturn 6, "Heheh.../%" (visible 8, length
      // 10), born f2034 with pos 2 and +1 a frame -- pos 9 at f2041 with halt
      // still 0, pos 10 AND halt 2 at f2042. The sim halted at pos > visible,
      // a frame early, and every talk phase ran short by that frame.
      if (!w.halted) {
        w.pos += 1;
        ...
        if (w.pos > visible + 1) w.halted = true;   // + 1 = the "/" frame (above)
      }
    ...
    if (state.dialogue.text) {
      // A truthy return is the guard case above: the death frame is over and
      // this step is the one that flips -- keep going into the selector.
    ...
    if (state.menu.fight.some(Boolean) && !e.bar) {
      ...
      // (kaizo/party/roster.js) range-guards the slot; the default keeps the
      // verified three-person behaviour untouched.
      ...
      //
      // ── THE MENU PHASE'S LENGTH IS THIS SCHEDULE, AND NOTHING ELSE ──────
      //
      // MEASURED on the _tok3 whole-fight pair, not reasoned. A menu phase is
      // exactly
      //
      //     nav + (lastBoltFrame + 38) + 1
      //
      // because every bolt is scored at `close == 13` (the token presses
      // confirm every other frame, so a bolt is taken on the first pressed
      // frame inside the `close < 15` window), the last character's
      // `attacked[]` latches one frame later, and `posttimer` then runs its 51.
      // Verified on both sides of every turn boundary in the recording.
      //
      // So a schedule difference is a TURN-BOUNDARY difference, frame for
      // frame — and the schedule is `choose()` off the GAME's live stream:
      //
      ...
      //
      // (That Create is byte-identical in vanilla and kaizo — only the Draw is
      // modded, adding a `kaizo_autocrit` branch that is dead: obj_knight_enemy
      // Create:111 sets `kaizo_autocrit = 0` and nothing else ever writes it.
      // `buildBolts` in sim/fightbar.js is a faithful translation of the block
      // above; the values it produces are simply a different draw.)
      //
      // CLAUDE.md, "LIVE RNG IS RE-ANCHORED PER ATTACK LAUNCH": the continuous
      // stream is deliberately not modelled, and these draws happen mid-menu,
      // thousands of draws from any anchor. So this cannot be translated into
      // agreement — it has to be REPLAYED, like the shuffle and the bolt
      // schedules the vanilla whole-fight tool already replays via `--bolts`.
      //
      // THE MOD'S SCHEDULES ARE RECOVERABLE FROM THE RECORDING WE ALREADY
      // HAVE. obj_attackpress's Draw trails each live bolt with an
      // obj_afterimage at
      //
      //     x = (x + 80 + boltframe[i] * boltspeed) - boltx * boltspeed
      //     y = y + 38 * boltchar[i]
      //
      // and the recorder logs every obj_afterimage's x/y in
      // `kaizo_oracle_seq_tok3.csv`. With x = 2, y = 365, boltspeed = 8, a
      // spawn on row y in {365, 403, 441} is a bolt trail: its row is the
      // character and `close = (x - 82) / 8`. boltx is 0 on the bar's first
      // Draw, so `boltframe = close + (frame - barCreatedFrame)`. Decoded, all
      // 32 bars of _tok3 read:
      //
      //     bar f272   0@29  2@41  1@41
      //     bar f682   1@29  0@41  2@59
      //     bar f1062 .. f12662   0@29     (one bolt, Kris, every turn after)
      //
      // and every bolt's last trail sits at close 13, on both sides, 32/32.
      //
      // CrescentSlash's menu is oracle 108 / sim 96 for exactly this: the mod
      // drew 29/41/59 and the sim drew 29/29/47, and 59 - 47 = 12. Feeding the
      // decoded table in through `state.boltSchedules` below takes that turn to
      // 427/427 with both phases exact, restores the turn-3 offset to 0, and
      // takes the whole fight from sum|drift| 733 / sum|MENU| 605 to 590 / 418
      // with the sim reaching the recording's 33 launches instead of 32.
      //
      // THE TABLE IS SUPPLIED NOW: `kaizo/tools/kaizo-trace.mjs --bolts <file>`,
      // fed by `kaizo/tools/decode-bolts.mjs` (which recovers the schedules
      // from the afterimage trails in the recording's seq CSV). With it, turn 1
      // and turn 2 are both EXACT on their turn boundaries — CrescentSlash's
      // −12 goes to 0.
      //
      // TWO TRAPS COST A RUN EACH GETTING THERE, both recorded so nobody pays
      // twice. (1) The bar MUTATES the schedule it is handed (`alive` clears as
      // bolts resolve) and `--sync auto` runs the tracer TWICE, so the table
      // must be DEEP-CLONED per run or the real run replays already-dead bolts,
      // never latches `attacked[]`, and hangs in the menu for the whole
      // recording. (2) The recording's bar 0 is its FIRST TURN, but the sim
      // opens with a synthetic lead-in that raises a bar of its own — consuming
      // from frame 0 hands every turn the previous turn's schedule and moves
      // the derived sync by 17 frames. `boltSkipBeforeFrame` is that fence.
      if (order.length) {
        // Replay the oracle's schedule when one was supplied — see
        // sim/fightbar.js. Consumed in creation order, one per bar.
        // A RECORDED SCHEDULE IS ONLY USABLE ON A MATCHING ROSTER.
        //
        // `createFightBar` takes `recorded` INSTEAD of generating, and
        // `attacked[i]` latches only when character i's last bolt is gone. So
        // a schedule that carries no bolt for a character the sim is fielding
        // leaves that character unlatched FOREVER and the menu never ends --
        // the whole run stalls in mnfight 0 with one launch.
        //
        // That is not hypothetical, it is what _tok3 does. The recording's
        // bars 2..31 carry ONE bolt (Kris, 29), because in the recording the
        // rest of the party was DOWN: the recorder pins HP but never revives,
        // and CLAUDE.md's "Restoring HP does not stand anyone up" is exactly
        // this -- being down is five globals and `chardead` is not one of the
        // ones a refill clears. The sim's keep-alive refills every frame, so
        // nobody ever reaches 0 and `chardead` is never set: it fields three.
        //
        // Until the roster is reconciled, replay the schedule only when its
        // character set matches the bar's, and say so when it does not. The
        // first two bars DO match (three bolts each), and bar f682 is the
        // whole of CrescentSlash's menu divergence, so this still buys the
        // measurement it was added for.
        // Bars raised before the sync frame belong to the sim's own lead-in and
        // have no counterpart in the recording -- see boltSkipBeforeFrame.
        const beforeSync = state.frame < (state.boltSkipBeforeFrame ?? 0);
        const rec = beforeSync ? null : state.boltSchedules?.[state.boltIndex];
        // Advance regardless: the index tracks BAR ORDER, not usage, and
        // skipping the increment would slide every later schedule by one.
    ...
    if (!e.balloonDone) {
      e.balloonDone = true;
      // THE ENEMY-TALK ENTRY, counted. obj_knight_enemy's enemytalk branch
      // ends with `rtimer = 0` (kaizo Step_0:354) and this block is the sim's
      // transcription of that branch, so this is where the recorder's
      // `rtimer` column resets. kaizo-trace.mjs watches the counter rather
      // than the clock: the mod's rtimer HOLDS AT 12 from launch through the
      // whole party phase and only this branch brings it back to 0 — deriving
      // it from "is the clock on" put a 0 in every menu frame where the
      // recording reads 12, which was the whole-fight gate's first divergence
      // (f672, column rtimer) for the entire life of the kaizo lane.
      ...
      if (state.gmlRng) {
        // THE DRAW STRUCTURE IS UNTOUCHED ON PURPOSE. `choose(0, 1, 2)` is
        // one u32 draw over THREE slots whatever the party size — the mod
        // does not narrow it, and narrowing it here would pick a different
        // target from the same draw and desync every later roll. What a
        // short roster changes is only the REROLL PREDICATE: slot 2 is not
        // targetable, so the loop spins until it lands on a live one,
        // exactly as the real game does with two members.
    ...
    if (e.spawnDelay > 0) {
      if (e.spawnDelay === RTIMER_SPAWN) {
        ...
        // THE ROW THE KNIGHT IS LEAVING IS THE PHASE HE REPORTS. Read the
        // pointer BEFORE the advance: the mod sets `phase = kaizo_AT.attackPhase`
        // in its selector (obj_knight_enemy Other_10:6), and the recording shows
        // that value lagging the attack label by exactly one row --- _tok3's
        // label turns to atk_Starstorm2 (table row 10, attackPhase 2) on f3720
        // with phase still 1, and phase only reaches 2 on f4045, as Quickslash
        // (row 11) is selected. The sim used to publish the pointer AFTER the
        // advance, so every phase boundary arrived a whole turn early: the
        // whole-fight gate's TURN group broke at f3709 on exactly that.
        const leavingPhase = e.phase;
        (e.hooks.advanceTurn ?? advanceTurn)(state);
        // ...UNLESS THE LANE HAS ITS OWN SELECTOR. The V-C director's `advance`
        // hook publishes the knight's phase itself (kaizo-vc-hooks.js) and
        // parks the row for the turn-end alarm; writing it again here, from a
        // pointer that has already moved, put a spurious flip on the arena-open
        // of every turn (sim f3709 and f3945 against the recording's single
        // f4045). The vanilla path has no such hook and still needs this.
        ...
        // their final turn, phase 4's third turn hands back to 3.
        {
          ...
          if (e.hooks.advance) {
            // THE MOD DOES NOT COUNT PHASES, IT READS THEM OFF THE ROW.
            // obj_knight_enemy Other_10:6 is `phase = kaizo_AT.attackPhase`,
            // and kaizo_AT here is the row that has just been playing -- so the
            // knight's phase lags the schedule by one row. MEASURED on _tok3:
            // the attack label turns to atk_Starstorm2 (table row 10,
            // attackPhase 2) on f3720 while phase still reads 1, and phase
            // reaches 2 only on f4045, as Quickslash (row 11) is selected.
            // The vanilla rule below -- flip at the selection of a phase's
            // LAST turn -- is right for the original's hardcoded selector and
            // wrong here; it moved every kaizo phase boundary a turn early and
            // broke the whole-fight gate's TURN group at f3709.
        ...
        // clock floors to 89.
        if (e.hooks.onSelect) {
          // V-C: apply the row's attackSetVar side effects at SELECTION,
          // exactly where the mod's Other_10 applies them. The vanilla
          // ROARING lines below are vanilla-schedule-specific.
    ...
    // THE KAIZO LEDGER — what actually launched, in order. verify-kaizo's
    // fight-order analog asserts against this rather than the HUD label,
    // because a label proves selection and the ledger proves launch.
...
export function buildKaizoTurnLoop(state, { seed = 12345, table, hooks = {} } = {}) {
  ...
  // (The old KAIZO_BOLTS env hook lived here. It parsed `char:frame`
  // while decode-bolts.mjs and --bolts both emit `frame:char`, so it was
  // a second differently-shaped parser for the same feed — deleted once
  // kaizo-trace.mjs owned the feature, rather than left to be found.)

  // Visual only — see sim/actors.js. None of these can touch bullet state.
  // Every position is measured from traces/flurry2.csv.
  // THE KAIZO KNIGHT for the mod-recreation versions: the same verified
  // actor, wrapped to add the mod's always-on rainbow afterimage trail
  // (obj_knight_enemy Create:38 `rgbafterimages = 1`). V-A keeps the vanilla
  // actor — its schedule is invented but its Knight is the real one.
  spawn(state, hooks.knightActor ?? knightActor, { x: KNIGHT.x, y: KNIGHT.ystart });
  // THE ROSTER IS NOT ALWAYS THREE. The Weird Route fields Kris + Noelle
  // (the mod's `global.char = [1, 4, 0]`, canonicalised by scr_fixparty), so
  // the actor table comes from the hook when one is supplied. Defaulting to
  // sim/actors.js's PARTY keeps every hookless version byte-identical.
