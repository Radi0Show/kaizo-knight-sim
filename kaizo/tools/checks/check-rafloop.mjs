...
// without permission.
//
//   node kaizo/tools/checks/check-rafloop.mjs
//
// WHY THIS REPO NEEDS ITS OWN COPY. `web/kaizo.js` is not vendored — it is this
// repo's own driver, forked from knight-sim's and carrying the same loop. The
// engine suite over there (tools/verify-rafloop.mjs) cannot see this file, and
// this is the page a player actually reported the problem on.
//
// The bug this exists to keep out, measured on the shipped kaizo driver in a
// real browser with a virtual monitor standing in for the compositor (the
// numbers are frame() entries per second, and the chain count is how many
// rAF(frame) requests are outstanding at once):
//
//   before a tab switch      61.3 frame()/s    1 chain
//   after one switch        483.0 frame()/s    8 chains
//   after two               917.2 frame()/s   15 chains
//   after three            1211.7 frame()/s   22 chains
//
// knight-sim's identical loop measured 60.1 / 490.5 / 886.8 / 1314.7 with
// chains 1 / 8 / 15 / 28 — the same mechanism, the same arithmetic.
//
...
// web/kaizo.js and RUNS it, against a virtual clock, a virtual monitor and a
...
const DRIVER = join(here, '..', '..', '..', 'web', 'kaizo.js');
...
console.log('frame() reschedules through exactly one place');
{
  ...
  ok(start >= 0, 'frame(now) found in web/kaizo.js');
  ...
  ok(routed.length >= 8, 'every exit path reschedules through scheduleFrame()', `${routed.length} call sites`);
}
...
const region = extractRegion(src, 'web/kaizo.js');
