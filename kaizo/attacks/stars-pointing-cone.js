// KAIZO obj_knight_pointing_cone — the driver of `Stars` as the mod rebuilds
// it: the turn-end recall only fires the stars flagged `starry == 1`; every
// other star is set `stay = 1` and lingers on the field instead of
// detonating.
//
// V-C recreation of EnderCat8's Kaizo Roaring Knight — do not publish without
// permission (kaizo/HANDOFF.md §5-C publish gate).
//
// PROVENANCE: copied from the VERIFIED sim/attacks/pointing-cone.js and
// changed ONLY where the mod's GML diverges from vanilla v105. Ground truth:
//   knight-research/kaizo-mod/gml_kaizo_dump/CodeEntries/
//     gml_Object_obj_knight_pointing_cone_Step_0.gml (the recall: l.43-69)
//     gml_Object_obj_knight_pointing_cone_Draw_0.gml (visual only)
//   delta specs: gml_Object_obj_knight_pointing_cone_Step_0.md / _Draw_0.md
//
// DIVERGENCES from the sim module:
//   * the release branch (`angle_lerp == 1`) filters on `starry == 1`
//     (Step l.48-57) — non-starry stars get `stay = 1` and are never given
//     `con = 1`; the stagger indices run over the starry subset only.
//   * NOT TRANSLATABLE FROM sim/ STATE — RENDERER WORK, and the ONE visual
//     delta in the whole Stars family that no image_blend can carry. Both
//     kaizo hunks change the NUMBER of draw calls, not a colour:
//       - Draw l.59-65: the additive flow-line streaks are each issued a
//         SECOND time, and the `(timer % 2)` gate on the fast streak is
//         bypassed by its unconditional duplicate — double brightness plus
//         a streak on every frame instead of every other one.
//       - Draw l.115-123: the cone interior's two scrolling background
//         layers run inside `repeat (2)` with `_bgsprite` walking
//         spr_knight_bullet_flow -> 4994 (spr_knight_bullet_flow_alt, a
//         mod-added sheet — now packed into kaizo/assets/sprites, so the
//         renderer has the art whenever it takes this on).
//     Neither draws RNG, so skipping them is stream-safe; there is simply
//     no instance field to put them in. render/draw/pointing-cone.js owns
//     this one, and render/ is outside the kaizo membrane. Flagged open.
//   * kaizo's own blue tints in this family DO land, just not here: the
//     charge/stay ramps are on the STAR (stars-pointing-star.js) and the
//     coltimer scheme is on the SHARD (stars-pointing-starchild.js).
//   * kaizo's Draw recolors (charge tint gray->#86A2FF) live on the STAR, not
//     here — see stars-pointing-star.js.
//
// Everything below not marked KAIZO is byte-identical to the sim module.
// ---------------------------------------------------------------------------
// (sim header, still true:)
// This is the object that shoves the battle box. Unlike most attacks it does
// not just add bullets: it drags the arena itself leftward every frame and
// squeezes the soul against the shrinking wall, which is the attack's real
// dodge pressure.
//
// Shape:
//   tween      ease into position beside the box over ~20 frames
//   open       angle eases 0 -> target_angle (60) via scr_ease_out(_, 6)
//   push       every frame: gt_x -= angle / target_angle / 2, then the box
//              snaps to round(gt_x) and the soul is clamped to its right edge
//   knockback  pulses shove the box harder for a few frames
//
// THE DRAW EVENT IS NOT PURELY COSMETIC: the cone's OPENING is driven from
// Draw, modelled in `endStep` below (this engine's equivalent slot).

import { scrMovetowards, scrEaseIn, scrEaseOut, lerp } from '../../sim/gml.js';
import { cue } from '../../sim/audio.js';
import { scrAfterimage } from '../../sim/fx.js';
import { gmlIrandom, gmlIrandomRange, gmlRandomRange } from '../../sim/rng.js';

/**
 * The end of the cone's Step, verbatim: knockback-or-drift, the two fake_gt
 * jitter draws (REAL stream consumption — the values are visual), the box
 * snap and the soul squeeze. Called exactly once per frame from both the
 * pre-con-2 path and the live path.
 */
function dragStep(e, state, gt, heart) {
  if (e.knockback !== 0) {
    const kb = scrEaseIn(e.knockback / 10, 5) * 10;
    e.gt_x -= kb;
    e.knockback = scrMovetowards(e.knockback, 0, 0.5);
    e.fakeGtXoff = gmlRandomRange(state.gmlRng, -1, 1) * (kb / 10);
    e.fakeGtYoff = gmlRandomRange(state.gmlRng, -1, 1) * (kb / 10);
  } else {
    e.gt_x -= e.angle / e.target_angle / 2;
    e.fakeGtXoff = gmlRandomRange(state.gmlRng, -1, 1) * (e.angle / e.target_angle);
    e.fakeGtYoff = gmlRandomRange(state.gmlRng, -1, 1) * (e.angle / e.target_angle);
  }
  // The box snaps to the integer position; the soul is squeezed against it.
  if (gt) gt.x = Math.round(e.gt_x);
  if (gt && heart) heart.x = Math.min(heart.x, gtMaxX(gt) - 22);
}

function box(state) {
  return state.entities.find((e) => e.alive && e.type.name === 'obj_growtangle');
}

/** gt_maxx() — the box's right edge. */
function gtMaxX(gt) {
  return gt.x + (gt.mask.w * gt.image_xscale) / 2;
}

export const pointingCone = {
  name: 'obj_knight_pointing_cone',
  // THE MIXED STEP ORDER, fitted to two exact measurements the pure orders
  // cannot both satisfy:
  //
  //  * frame 145 (the first star): size = us[38] of the anchored stream
  //    requires the CONTROLLER's rolls before the cone's two drag draws;
  //  * frame 160 (the squeeze release): the soul leaves the pinned clamp at
  //    365 = (new box clamp 369, applied first) - 4, requiring the cone's
  //    drag-and-clamp before the HEART's movement — while the heart is the
  //    OLDER instance.
  //
  // [dc, cone, heart] is the one order satisfying both. The same knob the
  // sword vortex already needs (its sword steps before its older manager);
  // GameMaker's real cross-object scheduling remains unexplained, these
  // measurements are not.
  stepOrder: -1,

  create(e, state) {
    // `obj_knight_enemy.visible = false` — the cone's Create HIDES THE KNIGHT,
    // because the cone draws its own pointing pose in his place. An invisible
    // instance's Draw event does not run at all in GameMaker, so for the whole
    // of Stars the real Knight contributes nothing to the screen.
    {
      const k = state.entities.find((x) => x.alive && x.type.name === 'obj_knight_enemy');
      if (k) k.visible = false;
    }
    // MEASURED from the recording, like the star's. Not in the GML dump.
    //
    // THE CONE IS THE KNIGHT during Stars. Its Draw calls `draw_self()`, and
    // its Create hides obj_knight_enemy — one figure on screen, in the
    // pointing pose, not two.
    e.sprite_index = 'spr_roaringknight_point_ol';
    // `image_speed = 0` — the pose is driven BY HAND in Draw, one frame at a
    // time (see endStep).
    e.image_speed = 0;
    e.image_index = 0;
    e.image_number = 5; // spr_roaringknight_point_ol
    const gt = box(state);
    e.angle = 0;
    e.target_angle = 60;
    e.tween = 0;
    e.angle_lerp = 0;
    e.image_xscale = 2;
    e.image_yscale = 2;
    // obj_knight_pointing_cone Create: `obj_knight_enemy.visible = false`, and
    // it inherits his afterimage timer so the trail does not restart.
    const knight = state.entities.find(
      (x) => x.alive && x.type.name === 'obj_knight_enemy',
    );
    if (knight) {
      knight.visible = false;
      e.aetimer = knight.aetimer ?? 0;
    } else {
      e.aetimer = 0;
    }

    e.afterimage_spread = 0;
    // `yoff = irandom(60) + 2` — where the charge beam samples the flow
    // texture, so the beam's pattern differs every time the attack runs.
    e.yoff = 2 + gmlIrandom(state.gmlRng, 60);
    e.con = 0;
    e.difficulty = 0;
    // ONE, not zero. The opening counter lives in the Draw event, and
    // GameMaker draws an instance on the frame it is created while its Step
    // does not run until the next one — the recording shows exactly that at
    // frame 13: `con` is already 1 (Draw ran and advanced it) while `tween` is
    // still 0 (Step had not). This engine has no Draw phase, so the cone's
    // endStep first runs a frame later; starting the counter at 1 puts it back
    // in step. Without it the cone opens one frame late and every angle after
    // it is shifted.
    e.timer = 1;
    e.timerb = 0;
    e.gt_x = gt ? gt.x : 320;
    e.knockback = 0;
    e.endtimer = 120;
    e.xstart = e.x;
    e.ystart = e.y;

    // KAIZO DRAW-TIME STATE the kaizo renderer reads (kaizo/render/draw/
    // pointing.js drawObjKnightPointingCone). These are REAL GML instance
    // fields of the cone that the sim never carried, because render/draw/
    // pointing-cone.js kept private per-paint substitutes for them (a WeakMap
    // toggle, an age-since-first-paint counter, `state.frame % 2`) —
    // substitutes that advance at the monitor's rate and start from the
    // first paint rather than from the GML's own values. Owned here so the
    // renderer reads state and owns none:
    //   Create l.5-6   bg_x = 0; lines_x = 0;  the interior's scroll offsets,
    //                  advanced by Draw l.124-133 on surface frames only
    //   Create l.29    star_flicker = 0;       the grate's 0/2 crawl (Draw
    //                  l.152-153, only while a star exists, l.139)
    //   Create l.30    draw_angle = 0;         the wedge's 1-degree jitter,
    //                  toggled at Draw l.102
    e.bg_x = 0;
    e.lines_x = 0;
    e.star_flicker = 0;
    e.draw_angle = 0;
    // ...AND WHAT THIS FRAME'S DRAW DREW. `timer`, `bg_x`, `lines_x` and
    // `star_flicker` are READ-THEN-WRITTEN by the Draw (drawn at their
    // current value, then stepped), while the renderer paints AFTER endStep
    // — where the Draw is modelled — and would otherwise see the stepped
    // values, one step ahead of the picture the game showed (a 20px texture
    // phase, a 2px grate crawl, the beam one slice early). endStep therefore
    // records the values its Draw model consumed, plus which branch it took
    // — the con >= 4 exit (l.38-41), the charge (l.50-80), the flare
    // (l.81-95) or the surface (l.96-171) — because `con` itself advances
    // INSIDE the Draw (l.54, l.77, l.92) and the post-Draw `con` cannot tell
    // a flip frame from the frame after it. Sim-side bookkeeping, not GML
    // fields; nothing in the sim reads them.
    //
    // The creation frame: the launcher stands in for the Draw GameMaker runs
    // on the frame of creation (stars-controller.js sets `con = 1`; `timer =
    // 1` above) — that Draw took the charge branch with timer 0.
    e.drawn_branch = 'charge';
    e.drawn_timer = 0;
    e.drawn_bg_x = 0;
    e.drawn_lines_x = 0;
    e.drawn_star_flicker = 0;
  },

  step(e, state) {
    const gt = box(state);
    const heart = state.soul;
    // NO SOUL, NO TARGET. obj_heart exists only during the bullet phase — the
    // Knight delivers it per turn via scr_moveheart and it is gone by the
    // party's menu — so a bullet that outlives its turn by a frame has
    // nothing to aim at. Skipping the frame leaves it where it was until the
    // turn sweep takes it; inventing a position would make it lunge at a soul
    // that is not there.
    if (!heart) return;
    e.timerb += 1;
    // THREE COPIES OF EACH, at the same pitch — the original stacks them to
    // thicken the sound rather than raising the gain, and a single cue is
    // noticeably thinner.
    if (e.timerb === 3) {
      for (let i = 0; i < 3; i++) cue(state, 'snd_knight_drawpower', 1.3);
    }
    if (e.timerb === 120) {
      for (let i = 0; i < 3; i++) cue(state, 'snd_knight_star_explosion_close', 0.7, 2);
    }

    if (e.con === 4) {
      // THE RETURN. `lerp(x, knight.x, 0.15)` each frame, so it eases home and
      // never quite arrives — the cone is cleared by the end-of-turn sweep,
      // not by reaching him.
      //
      // ORIGINAL BUG: `con = 5` — which restores the knight's visibility — is
      // guarded by `if (tween == 0)`. `tween` starts at 0, is only ever moved
      // TOWARD 1, and is never reset, so that test can never pass once the
      // cone has slid into place. con 5 is unreachable and the restore always
      // comes from CleanUp instead (the fight scene does the CleanUp's job).
      // Translated as-is rather than "corrected". (Kaizo keeps this verbatim.)
      if (e.tween === 0) {
        e.con = 5;
        // The same branch restores him: `obj_knight_enemy.visible = true`.
        // Unreachable for the reason above, translated as-is rather than
        // "corrected" — the restore really does come from CleanUp.
        const kv = state.entities.find((x) => x.alive && x.type.name === 'obj_knight_enemy');
        if (kv) kv.visible = true;
      }
      const knight = state.entities.find(
        (x) => x.alive && x.type.name === 'obj_knight_enemy',
      );
      if (knight) {
        e.x = lerp(e.x, knight.x, 0.15);
        e.y = lerp(e.y, knight.y, 0.15);
      }
    } else if (e.tween < 1) {
      e.tween = scrMovetowards(e.tween, 1, 0.05);
      const ease = scrEaseOut(e.tween, 4);
      if (gt) {
        e.x = lerp(e.xstart, gt.x + 115, ease);
        e.y = lerp(e.ystart, gt.y - 56, ease);
      }
    }

    // `if (con < 2) { exit; }` — the Step's own gate. The drag consumes RNG
    // only from con 2 (see the sim module's measured note).
    if (e.con < 2) return;

    if (state.turntimer <= e.endtimer) {
      // FIRE. On the first frame of the closing branch angle_lerp is still 1,
      // so the recall runs once. This is the moment the attack turns from
      // "accumulating" to "incoming".
      if (e.angle_lerp === 1) {
        // NEWEST FIRST. `with (obj_knight_pointing_star)` visits instances in
        // REVERSE creation order, so the stagger `timer = -i` lands on the
        // youngest star first and the oldest last (measured — see the sim
        // module and sim/entity.js; the same ordering shows up in the sword
        // vortex).
        //
        // KAIZO Step l.48-57: only stars with `starry == 1` are recalled
        // (con = 1, staggered); the rest get `stay = 1` and become lingering
        // hazards — their stay behaviour lives in the STAR's Draw
        // (stars-pointing-star.js endStep):
        //
        //     with (obj_knight_pointing_star) {
        //         if (starry == 1) { other.starlist[_count] = self;
        //                            con = 1; _count++; }
        //         else             { stay = 1; }
        //     }
        //
        // The stagger loop (`timer = -other.i`) runs over the starry subset
        // only, so the indices stay dense — modelled here by `count` advancing
        // only on recalled stars, exactly as the sim collapses the two GML
        // loops into one. (The GML stagger loop's odd-index
        // `playSound = false` is un-modelled in the sim copy too — audio-only.)
        const list = state.entities
          .filter((s) => s.alive && s.type.name === 'obj_knight_pointing_star')
          .reverse();
        let count = 0;
        for (const s of list) {
          if (s.starry === 1) {
            s.con = 1;
            s.timer = -count;
            count += 1;
          } else {
            s.stay = 1;
          }
        }
        e.knockback = 10;
      }

      // Closing: the angle eases back down.
      if (e.angle_lerp === 0 && e.con < 3) {
        e.timer = 10;
        e.con = 3;
        // Re-rolled for the closing flare.
        e.yoff = 120 + gmlIrandomRange(state.gmlRng, -60, 60);
      }
      e.angle_lerp = scrMovetowards(e.angle_lerp, 0, 0.1);
      e.angle = lerp(0, e.target_angle, scrEaseIn(e.angle_lerp, 6));
    } else if (e.angle < e.target_angle) {
      // Opening.
      e.angle_lerp = scrMovetowards(e.angle_lerp, 1, 0.025);
      e.angle = lerp(0, e.target_angle, scrEaseOut(e.angle_lerp, 6));
    } else {
      e.x += 0.25;
    }

    dragStep(e, state, gt, heart);
  },

  /** The opening AND the close-out, both driven from the original's Draw. */
  endStep(e, state) {
    // THE AFTERIMAGE TRAIL, every fourth frame while con <= 4. A ghost of the
    // pointing knight that drifts outward: `speed = 2 + afterimage_spread / 30`
    // and `direction = sin(aetimer) * angle / 2`, so it fans WIDER as the cone
    // opens (the direction scales with `angle`) and FASTER as `afterimage_spread`
    // grows. The spread climbs every trail frame from con 1 and is pulled back
    // to 0 at con 4, so the fan collapses as he lowers his arm.
    e.aetimer += 1;
    if (e.con <= 4 && e.aetimer % 4 === 0) {
      const ai = scrAfterimage(state, e);
      ai.image_alpha = 0.6;
      ai.fadeSpeed = 0.02;
      ai.builtinMotion = true;
      ai.speed = 2 + e.afterimage_spread / 30;
      ai.direction = (Math.sin(e.aetimer) * e.angle) / 2;
      const knight = state.entities.find(
        (x) => x.alive && x.type.name === 'obj_knight_enemy',
      );
      ai.depth = (knight ? knight.depth : 88) + 1;
      if (e.con === 4) e.afterimage_spread = scrMovetowards(e.afterimage_spread, 0, 20);
      if (e.con >= 1) e.afterimage_spread += 1;
    }

    // THE POSE, hand-animated in Draw rather than by image_speed:
    //
    //   con < 3   image_index += 0.5, up to the last frame, then it HOLDS —
    //             eight frames to raise the arm, and it stays pointed
    //   con >= 3  a single step back off the last frame, then -0.25 a frame
    //             as he lowers it
    //
    // Slow and one-directional in both halves. Nothing here ever wraps.
    const last = (e.image_number ?? 5) - 1;
    if (e.con >= 3) {
      if (e.image_index === last) e.image_index -= 1;
      else if (e.image_index > 0) e.image_index -= 0.25;
    } else if (e.image_index < last) {
      e.image_index += 0.5;
    }

    // KAIZO Draw l.38-41 `if (con >= 4) exit;` — nothing below runs while the
    // cone is on its way home. Recorded for the renderer (see Create); the
    // two blocks below never fired at con >= 4, so no state changes here.
    if (e.con >= 4) {
      e.drawn_branch = 'none';
      return;
    }

    if (e.con <= 1) {
      // Draw l.59-71: the charge beam is drawn with `timer` as it stands
      // HERE, before l.73's `timer++` — recorded for the renderer.
      e.drawn_branch = 'charge';
      e.drawn_timer = e.timer;
      if (e.con === 0) e.con = 1;
      e.timer += 1;
      if (e.timer >= 30) {
        // The charge beam finishing — Draw line 74, on the con 1 -> 2 flip.
        cue(state, 'snd_rocket_long', 0.6);
        e.timer = 0;
        e.con = 2;
      }
      // Draw l.79 `exit;` — the surface block below is not reached on a
      // charge frame, the flip frame included: the surface first draws on
      // the frame AFTER con becomes 2. (The flare block below could not have
      // run here anyway — con is at most 2 now — so the return changes only
      // what is recorded.)
      return;
    }

    // THE CLOSE. Draw counts `timer` down from 10 once con reaches 3 (the
    // flare that plays as the cone snaps shut) and hands over to con 4, which
    // walks the cone back to the knight.
    if (e.con === 3 && e.timer > 0) {
      // Draw l.84-87: the flare's alpha is `timer / 10` with `timer` as it
      // stands HERE, before l.88's `timer--` — recorded for the renderer.
      e.drawn_branch = 'flare';
      e.drawn_timer = e.timer;
      e.timer -= 1;
      if (e.timer === 0) e.con = 4;
      // Draw l.94 `exit;`.
      return;
    }

    // THE OPEN CONE — KAIZO Draw l.96-171, the surface block, reached only at
    // con 2 (con 3 always arrives with timer 10 from the Step, l.74-75, so
    // it takes the flare above, and the flare hands straight to con 4). Its
    // instance-state writes, in the GML's order — the drawing itself is
    // kaizo/render/draw/pointing.js:
    e.drawn_branch = 'surface';
    // l.102 `draw_angle = 1 - draw_angle;` — toggled BEFORE l.103 reads it,
    // so the renderer takes the post-toggle value directly.
    e.draw_angle = 1 - e.draw_angle;
    // l.118-121 draw the two flow sheets at the CURRENT bg_x / lines_x...
    e.drawn_bg_x = e.bg_x;
    e.drawn_lines_x = e.lines_x;
    // ...then l.124-133 scroll them: `lines_x -= 80; bg_x -= 20;`, each
    // wrapped by 640 (the sheets are 320 wide, drawn at scale 2).
    e.lines_x -= 80;
    e.bg_x -= 20;
    if (e.lines_x < -640) e.lines_x += 640;
    if (e.bg_x < -640) e.bg_x += 640;
    // l.139 `if (instance_exists(obj_knight_pointing_star))` gates the star
    // surface; inside it l.152 draws the grate at the CURRENT star_flicker
    // and l.153 flips it (`star_flicker = 2 - star_flicker`). No star, no
    // flip — the crawl pauses with nothing to cut. ANY live star counts, not
    // only the accumulating con-0 ones — the `with` has no filter.
    if (state.entities.some((x) => x.alive && x.type.name === 'obj_knight_pointing_star')) {
      e.drawn_star_flicker = e.star_flicker;
      e.star_flicker = 2 - e.star_flicker;
    }
  },
};
