...
// ---- UNUSED OPENS THE KAIZO FIGHT -----------------------------------------
// The row was a reserved stub that played the error sound; it now hands the
// driver an intent to navigate to web/kaizo.html. Like SHARE it is NOT a page
// — confirming must stay on the hub and let the driver do the leaving, since
// sim/ has no DOM.
//
// The intent has to survive the trip out of stepSettings. stepTitle rebuilds
// the settings result from a WHITELIST of fields, and a new intent that is
// not added there is dropped silently: the row reports `selected`, the driver
// sees nothing to act on, and the button looks dead while every unit test of
// stepSettings passes. That is exactly what happened, so it is asserted here
// through stepTitle rather than against stepSettings directly.
{
  const t = createTitle();
  for (let i = 0; i < extraAt('settings'); i++) tap(t, 'down');
  tap(t, 'confirm');
  const row = SETTINGS_PAGES.findIndex((p) => p.id === 'unused');
  check(row >= 0, 'UNUSED should be in the settings hub');
  for (let i = 0; i < row; i++) tap(t, 'down');

  const r = tap(t, 'confirm');
  check(r.kaizo === true, 'confirming UNUSED should return kaizo:true THROUGH stepTitle');
  check(r.error !== true, 'and must no longer report an error — it is not inert any more');
  check(t.settings.page === null, 'and STAY on the hub — it is not a page');

  // Walking onto the row must not fire it, same as SHARE.
  const t2 = createTitle();
  for (let i = 0; i < extraAt('settings'); i++) tap(t2, 'down');
  tap(t2, 'confirm');
  let fired = false;
  for (let i = 0; i < row; i++) { if (tap(t2, 'down').kaizo) fired = true; }
  check(!fired, 'moving the cursor onto UNUSED must not launch anything');

  // And NOTHING ELSE in the hub claims it — a stray `kaizo` on another row
  // would send a player who picked GRAPHICS to a different fight.
  for (const p of SETTINGS_PAGES) {
    if (p.id === 'unused') continue;
    const t3 = createTitle();
    for (let i = 0; i < extraAt('settings'); i++) tap(t3, 'down');
    tap(t3, 'confirm');
    const at = SETTINGS_PAGES.findIndex((q) => q.id === p.id);
    for (let i = 0; i < at; i++) tap(t3, 'down');
    check(tap(t3, 'confirm').kaizo !== true, `${p.name} must not return the kaizo intent`);
  }
}
