<!-- Copied from knight-sim/docs/HANDOFF.md on 2026-09-02 (commit 247b1ba, working tree).
     REFERENCE ONLY: it describes the vanilla project this engine came from.
     Read on demand; CLAUDE.md here says which parts apply. -->
...
**55 suites, 1 failing** — `verify-fullfight` on the second token only; `verify21j` in the same suite is one-to-one. Left visible on purpose: it is the generalization front, not a regression. The whole-fight diff — the point of the project — passes; see §3 for what "one-to-one" precisely claims.
...
| whole-fight diff vs the oracle | **SOLVED 2026-08-22 — ALL FIVE FIGHTS PASS, AND THE ENDING IS NOW IN SCOPE.** `fullfight-end37` runs 12,126 frames, 119 past where every earlier recording stopped: the recorder used to halt the instant `end_cutscene_version` went positive, which is exactly the frame the END CUTSCENE BEGINS, so that whole section sat outside every trace by construction. The oracle patch now keeps a 120-frame tail and logs the cutscene's own state to `oracle_end.csv`; `verify-fullfight`'s `reportEnd` diffs it beside the camera. It found SEVEN divergences, every one measured: endtimer one high (ecv is set in the DRAW, endtimer++ is in the STEP, which has already run); `hurttimer` floored at 0 where `scr_enemy_hurt` runs it NEGATIVE, which froze the Knight's strobe on the idle frame for the whole ending; `snd_knight_hurtb` playing during the ending (its test carries `&& end_cutscene_version == 0`); the one-frame `stronghurtanim` blip at hurttimer 15; the white fadeout at HALF speed (`scr_fadeout(15)` is 1/15), unclamped past 1.0, with an f64 `fadespeed` accumulating into an f32 BUILT-IN; the attack bar frozen forever instead of frozen then DESTROYED; and the tension bar's exit decelerating instead of accelerating, from the wrong x, never dying. **Residual, deliberately unsolved:** `tb_x`/`tb_hspeed` drift ~2e-6 -- GameMaker derives `hspeed` from speed/direction and neither a pure-f32 nor a pure-f64 model reproduces the series; `obj_tensionbar`'s Draw exits on `ecv > 0` so it is provably invisible, and it is asserted with a tolerance rather than hidden. **Method note that made this section findable at all: a recording that stops at a boundary cannot distinguish 'holds forever' from 'holds then stops' -- the frozen attack bar had been FITTED to exactly that blind spot.** `verify21j`/`probe21` (12,011 frames) and `verify37`/`probe37` (12,007 frames), 176 columns, and verify37's camera matches on every frame. **Read the premise before quoting this as "one-to-one": it holds under FOUR measured micro-tolerances** (POSITION 0.05px, ANGLE 0.02deg, SCALE 5e-5, SOUL 0.01px) plus pinned equip arrays and FOUR replay feeds (`--shuffle --bolts --grazes --shards`). A raw tolerance-free scan still finds ULP-scale differences (~1.9e-6) on about 2,000 frames, from a constant one-ulp offset that first appears at f265/f221 and does NOT accumulate. **It is not bit-exact and must not be described as such.** The chase, for the record: f577 -> f1625 -> f2315 -> f3169 -> f3199 -> f4399 -> f6832 -> f11269 -> f11726 -> zero. The last four fixes: (a) the `SNAP_EPS` 1e-4 -> 1e-12 tightening, whose first pass was applied by regex and MISSED the gravity pair `gsn`/`gc` -- the half-fix was itself the next divergence; (b) TWO missing screen shakes, each invisible because no traced column held `camerax()` -- `obj_heroparent`'s Susie impact shake wrote a field nothing read, and `scr_damage_maxhp` creates its own shake without routing through `scr_damage`; (c) the charge-up's white afterimage `random(360)`, found by an ORACLE PROBE after four sessions of static analysis failed to attribute it; (d) the roar's pull reading the CLAMPED heart -- `obj_knight_roaring2_Step` opens with `with (obj_heart) { clamp to camera }` before its tempdir read, so on a SHAKEN frame the pull aims from the clamped position, not the frame-start one. **Method lessons worth more than the fixes, all of which cost multiple sessions:** (1) verify-fullfight's reported "first divergence" is a TOLERANCE FRONT, not an origin -- ALWAYS raw-scan before chasing it; f4399 was chased for sessions while the real origin sat 4,000 frames earlier. (2) A stale sim trace is not evidence; verify-fullfight now refuses one older than `sim/`. (3) Tokens are REGENERABLE -- every recording used `make-replay-token.mjs`'s defaults, so `--seed N --frames 20000` reproduces any of them; all four are saved at `traces/fullfight-<name>.token`. Try the generator before booking the machine for a re-record. (4) When static analysis stalls, PROBE THE ORACLE -- the needle-substitution idiom (read the decompiled entry, insert one guarded block) is safe, and its main trace came back byte-identical before the probe frame. (5) One diagnostic COLUMN broke f11726 open after three wrong theories: the soul's x is pinned at 0 for the whole roar except on the one shaken frame. **Tried and REJECTED, do not retry:** `viewFor(state, e)` in place of the global `state.view` -- twice, on `obj_regularbullet`'s wall cull and on the roar's pull target, each time breaking fights that were already exact; feeding the pull the LIVE heart instead of `soulPrev` (breaks verify21j at f11271); modelling the tunnel's `sin` with the f32 runner-trig family (broke `verify-tunnel-difficulty`, which is oracle-verified). **STILL OPEN, but NOT what it looked like:** the chargeup draw needs `chargeuptimer - 1`, and the hunt for "the one frame where the charge-up starts late" is CLOSED -- there is no such frame. Three eliminations, logged in full at the site in `sim/scenes/fight.js`: (1) moving the arm from the turntimer-arm frame to the dispatch frame, which is where the game really sets `chargeupcon = 1` (Step line 532), BREAKS ALL FOUR FIGHTS at f11027 column `menu` -- the `chargeuptimer == 60` stomp is what ends that turn, so the oracle's own menu frame pins the game's charge-up to the arm frame and its timer at the roar launch to 185; (2) the game never skips a tick -- the Step's one early exit needs a hero object to be missing and the heroes are created once in obj_battlecontroller's Create and never destroyed; (3) a delayed roar Create cannot split the tokens, since any delay that gives 37 a `% 4` draw gives 21 one too. **So at 185 the afterimage block cannot fire, and the extra u32 draw the probe measured is NOT the afterimage.** `- 1` encodes "timer mod 4 == 1", true on 37 and false on 21 -- load-bearing, so do not remove it without a replacement, but not the mechanism it names. Since it is the only single-draw call in the knight's Step below the selector, the draw belongs to ANOTHER object stepping between the knight and obj_dbulletcontroller. **NEXT:** probe the oracle for `chargeuptimer` and the stream position TOGETHER on the launch frame -- one recording, the idiom that cracked f11269. Also still worth doing: the four micro-tolerances were fitted to token 21's envelopes; re-measure them against token 37 now that both pass. |
