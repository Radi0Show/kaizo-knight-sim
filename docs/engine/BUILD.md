<!-- Copied from knight-sim/docs/BUILD.md on 2026-09-02 (commit 247b1ba, working tree).
...
`ORIGINAL BUG:` markers and the "tried and reverted" notes in `sim/`, `kaizo/`,
...
Current result: **157 files stripped, 24,946 comment/blank lines removed,
3.0 MB → 1.3 MB of text, 57% smaller.**

## What goes in

`web/ sim/ render/ input/ assets/ kaizo/`, mirrored exactly — every import in
here is relative (`../sim/`, `../../sim/`), so the layout cannot be flattened.

`kaizo/` is in the set because `web/kaizo.html` is a real second page:
`web/kaizo.js` imports `../kaizo/scenes/kaizo-fight.js`.

Left out: `kaizo/tools/` (verification machinery no browser requests), `*.md`,
dotfiles, and anything the publish gate withholds.
...
The answer is already written down, in git. `kaizo/assets/.gitignore` gates all
**446** files under `kaizo/assets/sprites/` — EnderCat8's own art for Kaizo
Roaring Knight, which per `kaizo/HANDOFF.md` §5-C may not be published without
their permission, plus the manifest and the `masks.json` derived from it.
...
A consequence worth knowing: `node kaizo/tools/verify-kaizo.mjs` run *against a
build output* fails two assertions — `check-sprites`' "the kaizo sprite overlay
exists" and `check-weirdroute`' "`kaizo/party/WEIRD-ROUTE.md` exists". Both are
assertions about the **source repo**, not about behaviour, and a served build is
supposed not to satisfy them. Run that suite against the source tree.
